$(document).ready(function($) {
    ...
    function loadData(parameters) {
        $.ajax({
            url: "https://planeacion.test/api/instituciones",
            dataType: "json",
            method: "GET",
            cache: false,
    ...
    */
    ...
    function createMarkers(){
        ...
        for (var i = 0; i < loadedMarkersData.length; i++) {
            ...
            ( ( loadedMarkersData[i]["Tipo"] !== undefined ) ? '<div class="ts-marker__image ts-black-gradient" style="background-image: url(assets/img/escudo_web.png)"></div>' : '<div class="ts-marker__image ts-black-gradient" style="background-image: url(assets/img/escudo_web.png)"></div>' ) +
            '</a>';
    ...
    function placeRichMarker(parameters){
        ...
        google.maps.event.addListener(marker, 'click', (function(marker, i) {
            return function() {
                ...
                openInfobox({"id": $(this.content.firstChild).attr("data-ts-id"), "parentMarker": this, "i": i, "url": "https://planeacion.test/api/instituciones" });
        ...
        */
        ...
        '<div class="ts-infobox" data-ts-id="'+ loadedMarkersData[i]["id"] +'">' +
            ...
            '<a href="'+ loadedMarkersData[i]["url"] +'" class="ts-infobox__wrapper ts-black-gradient">' +
                ( ( loadedMarkersData[i]["badge"] !== undefined && loadedMarkersData[i]["badge"].length > 0 ) ? '<div class="badge badge-dark">'+ loadedMarkersData[i]["badge"] +'</div>' : '<div class="badge badge-dark">Cupos disponibles</div>' ) +
                ...
                '</div>' +
                '<div class="ts-infobox_image" style="background-image: url(assets/img/IMG-20200229-WA0049.jpg)"></div>' +
            '</a>' +
        '</div>';
    }
            ...
            resultsHtml.push(
                '<div class="ts-result-link" data-ts-id="' + loadedMarkersData[id]["id"] + '" data-ts-ln="' + newMarkers[id].loopNumber + '">' +
                    ...
                    '<a href="'+ loadedMarkersData[id]["url"] +'" class="card ts-item ts-card ts-result">' +
                        ( ( loadedMarkersData[i]["ribbon"] !== undefined ) ? '<div class="ts-ribbon">'+ loadedMarkersData[i]["ribbon"] +'</div>' : "<div class='ts-ribbon'><i class='fa fa-thumbs-up'></i></div>" ) +
                        ( ( loadedMarkersData[i]["ribbon_corner"] !== undefined ) ? '<div class="ts-ribbon-corner"><span>'+ loadedMarkersData[i]["ribbon_corner"] +'</span></div>' : "" ) +
                        '<div href="detail-01.html" class="card-img ts-item__image" style="background-image: url(assets/img/IMG-20200229-WA0049.jpg)"></div>' +
                        '<div class="card-body">' +
                            '<div class="ts-item__info-badge">'+ loadedMarkersData[id]["Telefono"]+'</div>' +
                            '<figure class="ts-item__info">' +
